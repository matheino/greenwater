...
# Calculate and irrigated area factor to scale the harvested irrigated area data relative to irrigated areas in year 2000.
    ...
    irrig_area_factor[np.isnan(irrig_area)] = 1.0
...
# Writes raster yield data into netcdf.
    ...
    os.chdir(r'D:\work\research\greenwater\iizumi_yield_analysis\results\march2020')
...
# Import data
    ...
    gw_combined_mirca = np.zeros_like(gw).astype(bool)
    gw_combined_yield = np.zeros_like(gw).astype(bool)

    gw_anom_combined_mirca = np.zeros_like(gw_anom).astype(bool)
    gw_anom_combined_yield = np.zeros_like(gw_anom).astype(bool)
    
    for crop in crops:

        gw_data_mirca[crop] = np.copy(gw)
        gw_data_yield[crop] = np.copy(gw)

        gw_anom_data_yield[crop] = np.copy(gw_anom)
        gw_anom_data_mirca[crop] = np.copy(gw_anom)
...
# Change cells that are not considered rainfed to nan
        ...
        gw_data_mirca[crop][rfc_boolean == False] = np.nan
        gw_data_yield[crop][np.isnan(crop_yield_data[crop])] = np.nan
        
        gw_anom_data_mirca[crop][rfc_boolean == False] = np.nan
        gw_anom_data_yield[crop][np.isnan(crop_yield_data[crop])] = np.nan
        
        gw_combined_mirca[rfc_boolean] = True
        gw_combined_yield[~np.isnan(crop_yield_data[crop])] = True   

        gw_anom_combined_mirca[rfc_boolean] = True
        gw_anom_combined_yield[~np.isnan(crop_yield_data[crop])] = True

# Export data
        export_data(crop_yield_data[crop], crop, years, 'rainfed_yield')
        
        export_data(gw_data_mirca[crop], crop, years, 'green_water_mirca_mask')
        export_data(gw_anom_data_mirca[crop], crop, years, 'green_water_anomaly_mirca_mask')
    ...
    gw_anom_mirca = np.copy(gw_anom)
    gw_anom_yield = np.copy(gw_anom)
    
    gw_mirca[gw_combined_mirca == False] = np.nan
    gw_yield[gw_combined_yield == False] = np.nan
        
    gw_anom_mirca[gw_anom_combined_mirca == False] = np.nan
    gw_anom_yield[gw_anom_combined_yield == False] = np.nan
    
    export_data(gw_mirca, 'all_crops', years, 'green_water_mirca_mask')
    export_data(gw_yield, 'all_crops', years, 'green_water_yield_mask')
    export_data(gw_anom_mirca, 'all_crops', years, 'green_water_anomaly_mirca_mask')
    export_data(gw_anom_yield, 'all_crops', years, 'green_water_anomaly_yield_mask')
